...
<div class="flex-col w-full bg-gray-100 md:flex md:flex-row md:min-h-screen dark:bg-gray-900">
    ...
    <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-white md:w-64 dark:text-gray-200 dark:bg-gray-800" x-data="{ open: false }">
      ...
      <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4 mb-2 shadow-md bg-gray-50 dark:bg-gray-900 shadow-black">
        ...
        <a href="{{ route('profil') }}" class="flex gap-3 text-lg font-semibold tracking-widest text-gray-900 uppercase transition-colors duration-300 rounded-lg dark:text-white hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none focus:shadow-outline">
      ...
      <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
        <a class="flex gap-3 px-4 py-2 mt-2 text-sm font-semibold text-gray-900 transition-colors duration-300 bg-gray-200 rounded-lg dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-300 focus:bg-gray-300 focus:outline-none focus:shadow-outline" href="{{ route('adminpage') }}">  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="flex justify-between gap-4 text-center size-6">
        ...
        <div @click.away="open = false" class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg">
                <div class="px-2 py-2 bg-white rounded-md shadow dark:bg-gray-800">
                    <a class=" flex gap-3 px-4 py-2 mt-2 text-sm font-semibold text-gray-900 transition-colors duration-300 bg-transparent rounded-lg dark:bg-transparent dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="{{ route('show_book') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    <!-- Main Content -->
        <header class="flex items-center justify-between px-6 py-5 ml-4 mr-4 bg-gradient-to-t from-sky-500 to-sky-300 text-sky-800">
            <form action="{{ route('search') }}" method="get" class="flex items-center max-w-lg mx-auto space-x-4">
                ...
                <div class="relative flex-grow">
                    <input type="text" id="search-navbar" name="search" class="block w-full p-2 pl-10 text-sm text-gray-900 transition-all duration-300 border border-gray-300 rounded-lg search-input bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Recherchez un livre">
                ...
                <button type="submit" class="w-auto px-4 py-2 text-sm text-white transition-all duration-300 bg-gradient-to-t from-slate-400 to-slate-300 border border-transparent rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <!-- Hero Section -->
        <div class="relative py-12 mx-auto mt-5 text-left text-white rounded-md shadow-md bg-custom md:w-4/5 lg:w-4/5 xl:w-5/6">
            ...
            <div class="relative z-10 px-10">
                <div class="mx-auto">
                    ...
                    <p class="mb-4 text-sm">Découvrez les meilleurs livres pour enrichir votre esprit et mieux comprendre le monde.</p>
                    <div class="flex justify-center">
                        <a href="your-link-here.html" class="inline-block px-6 py-3 text-base font-semibold text-white bg-blue-500 rounded-lg hover:bg-gradient-to-r from-blue-400 to-blue-600">Explorer les livres</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container p-6 mx-auto">
            <h2 class="mb-4 text-2xl font-bold text-center text-transparent py-7 bg-gradient-to-tr from-sky-500 to-sky-400 bg-clip-text">Livres Disponible</h2>
    
           
    
            <div class="grid grid-cols-1 gap-4 px-4 shadow-md sm:grid-cols-2 lg:grid-cols-2 w- 1/3 shadow-black py-7">
                
                @foreach ($livre as $livre)
                    <div class="flex flex-col overflow-hidden bg-white rounded-lg shadow-md w-72 lg:flex-row shadow-slate-700">
                        <img class="object-cover pt-2 pl-3 rounded-sm w-fit h-36 lg:w-1/3" src="/storage/{{ $livre->book_img }}" alt="{{ $livre->title }}"> 
        ...
        <h2 class="mb-4 text-2xl font-bold text-center text-transparent py-7 bg-gradient-to-tr from-sky-500 to-sky-400 bg-clip-text">category currently available</h2>
